...
export const Leaves: React.FC = () => {
  ...
  const handleAction = async (id: string, action: 'TEACHER_APPROVED' | 'REJECTED') => {
    try {
      await api.put(`/leaves/${id}/review`, { status: action, remarks });
      setSelectedLeave(null);
      setRemarks('');
      fetchLeaves();
      alert(`Leave request recommended status updated successfully.`);
    } catch (e) {
      alert('Error updating leave status');
    }
  };

  if (loading) return <div>Loading leave logs...</div>;

  return (
    <div className="space-y-6">
      {/* Title */}
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight">Leave Recommendations</h1>
        <p className="text-muted-foreground text-sm">Review leave requests from your department and recommend approval to the Admin</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              >
                ...
                <div className="mt-4 md:mt-0 flex items-center gap-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold uppercase ${
                    ...
                    {leave.status === 'TEACHER_APPROVED' ? 'Recommended (Pending Admin)' : leave.status}
                  </span>
                </div>
              </div>
            ))
        ...
        <div className="bg-card border border-border p-6 rounded-2xl shadow-sm h-fit">
          <h3 className="font-bold text-lg mb-4">Leave Action</h3>
          {selectedLeave ? (
            <div className="space-y-4">
              ...
              {selectedLeave.status === 'PENDING' ? (
                <div className="space-y-3 pt-4 border-t">
                  <label className="text-xs font-semibold text-muted-foreground block">Recommend Remarks</label>
                  <textarea
                    rows={3}
                    placeholder="Enter recommendation remarks for the Admin..."
                    value={remarks}
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => handleAction(selectedLeave.id, 'TEACHER_APPROVED')}
                      className="flex-1 py-2 rounded-lg bg-primary text-primary-foreground font-semibold hover:bg-primary/95 flex items-center justify-center gap-1"
                    >
                      <CheckCircle size={16} /> Recommend
                    </button>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-muted-foreground italic border-t pt-4">
                  This leave request has already been reviewed ({selectedLeave.status.toLowerCase()}).
                </p>
              )}
            </div>
          ) : (
            <p className="text-sm text-muted-foreground italic">
              Select a leave request to recommend or decline the progressive workflow.
            </p>
          )}
        </div>
      </div>
